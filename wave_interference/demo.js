var demo = new Demo({
    ...
    init: function() {
        ...
        this.svg = d3.select("#demo").append("svg").attr("width", this.width + this.margin.left + this.margin.right).attr("height", this.height + this.margin.top + this.margin.bottom).append("g").attr("transform", "translate(" + this.margin.left + "," + this.margin.top + ")");
        this.path1 = this.svg.append("path").attr("stroke", "blue").attr("fill", "none");
        this.path2 = this.svg.append("path").attr("stroke", "red").attr("fill", "none");
        this.path12 = this.svg.append("path").attr("stroke", "rgb(150, 66, 180)").attr("fill", "none");
        this.tScale = d3.scale.linear().domain([0, this.dataLength - 1]).range(this.waveHorizontalExtent);
        this.canvasXScale = d3.scale.linear().domain(this.waveHorizontalExtent).range([0, this.width]);
        this.yScale = [0, -14];
        this.canvasYScale = d3.scale.linear().domain(this.yScale).range([0, this.height]);
        this.xAxis = d3.svg.axis().scale(this.canvasXScale).orient("bottom").ticks(10)
        this.yAxis = d3.svg.axis().scale(this.canvasYScale).orient("left").ticks(14).tickFormat("");
        this.svg.append("g").attr("class", "grid").attr("transform", "translate(0," + this.height + ")").call(d3.svg.axis().scale(this.canvasXScale).ticks(10).tickSize(-this.height)).selectAll(".tick").data(this.canvasXScale.ticks(10), function(d) {
            return d;
        }).exit().classed("minor", true);
        this.svg.append("g").attr("class", "x axis").attr("transform", "translate(0," + this.height + ")").call(this.xAxis);
        this.svg.append("g").attr("class", "y axis").call(this.yAxis.tickSize(-this.width));
        this.svg.append("text").attr("class", "x label").attr("text-anchor", "middle").attr("x", this.width / 2).attr("y", this.height + 50).text("Time in seconds");
        this.previousFrequency = 1;
        this.lineFunction = d3.svg.line().x(function(d) {
            return this.canvasXScale(d.x);
        }).y(function(d) {
            return this.canvasYScale(d.y);
        }).interpolate("linear");
        this.update();
    ...
    update: function(e) {
        ...
        if (e == "overlay") {
            ...
            this.path2.transition().duration(500).attr("transform", "translate(0 " + waveMargin + ")")
            this.path1.transition().duration(500).attr("transform", "translate(0 " + -waveMargin + ")")
        }
    },
    ...
    updateAxes: function() {
        ...
        this.svg.selectAll(".grid").call(d3.svg.axis().scale(this.canvasXScale).ticks(10).tickSize(-this.height)).selectAll(".tick").data(this.canvasXScale.ticks(10), function(d) {
            return d;
        }).exit().classed("minor", true);
    },
    ...
    resizeSVG: function() {
        ...
        this.updateAxes();
        d3.select(".y.axis").call(this.yAxis.tickSize(-this.width));
        d3.select(".x.label").attr("x", this.width / 2)
        this.updatePathData();
    }
});
